...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Text(80, 54, mb_convert_encoding('Boleta de Colaborador Derivado', 'ISO-8859-1', 'UTF-8'));
...
$id = $_GET['id'];
$stmt = $connect->prepare("SELECT derivados.id_der, derivados.fecha, derivados.idpa,  patients.idpa, patients.numhs, patients.nompa, patients.apepa, patients.grup, derivados.causa, derivados.obs FROM derivados INNER JOIN patients ON derivados.idpa = patients.idpa WHERE derivados.id_der= '$id'");
...
while ($row = $stmt->fetch()) {
    ...
    // Sección de 'Causa'
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(40, 7, mb_convert_encoding('Causa:', 'ISO-8859-1', 'UTF-8'), 1, 0, 'L', 0);
    $pdf->SetFont('Arial', '', 10);
    $pdf->Cell(150, 7, mb_convert_encoding($row['causa'], 'ISO-8859-1', 'UTF-8'), 1, 1, 'L', 0);

    // Sección de 'Observaciones'
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(40, 7, mb_convert_encoding('Observaciones:', 'ISO-8859-1', 'UTF-8'), 1, 0, 'L', 0);
    $pdf->SetFont('Arial', '', 10);
    $pdf->Cell(150, 7, mb_convert_encoding($row['obs'], 'ISO-8859-1', 'UTF-8'), 1, 1, 'L', 0); // Completa la fila
    ...
    $fileName = 'colaborador_derivado_' . $row['nompa'] . ' ' . $row['apepa'] . '_' . $row['numhs'] . '.pdf';
}
