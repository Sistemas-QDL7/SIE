...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Text(80, 54, mb_convert_encoding('Boleta de Entrega de Material', 'ISO-8859-1', 'UTF-8'));
...
$id = $_GET['id'];
$stmt = $connect->prepare("SELECT material.id_mat, material.fecha, material.idpa,  patients.idpa, patients.numhs, patients.nompa, patients.apepa, patients.grup, material.material, material.motivo FROM material INNER JOIN patients ON material.idpa = patients.idpa WHERE material.id_mat= '$id'");
...
while ($row = $stmt->fetch()) {
    ...
    // Sección de 'Material Entregado'
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(40, 7, mb_convert_encoding('Material Entregado:', 'ISO-8859-1', 'UTF-8'), 1, 0, 'L', 0);
    $pdf->SetFont('Arial', '', 10);
    $pdf->Cell(150, 7, mb_convert_encoding($row['material'], 'ISO-8859-1', 'UTF-8'), 1, 1, 'L', 0);

    // Sección de 'Motivo'
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(40, 7, mb_convert_encoding('Motivo:', 'ISO-8859-1', 'UTF-8'), 1, 0, 'L', 0);
    $pdf->SetFont('Arial', '', 10);
    $pdf->Cell(150, 7, mb_convert_encoding($row['motivo'], 'ISO-8859-1', 'UTF-8'), 1, 1, 'L', 0); // Completa la fila
    ...
    $fileName = 'entrega_de_material_' . $row['nompa'] . ' ' . $row['apepa'] . '_' . $row['numhs'] . '.pdf';
}
