...
<head>
    ...
    <title>   SIE QDL | Información de la cita</title>
</head>
<body>
...
require '../../backend/bd/Conexion.php';
 $id = $_GET['id'];
 $sentencia = $connect->prepare("SELECT orders.idord, orders.id_consulta, orders.total_products, orders.total_price, orders.placed_on, events.id, events.title, patients.idpa, patients.numhs,patients.nompa, patients.apepa, patients.grup, doctor.idodc, doctor.ceddoc, doctor.nodoc,doctor.nomesp, doctor.apdoc, laboratory.idlab, laboratory.nomlab, events.start, events.end, events.color, events.state,events.chec,events.monto FROM events INNER JOIN patients ON events.idpa = patients.idpa INNER JOIN doctor ON events.idodc = doctor.idodc INNER JOIN laboratory ON events.idlab = laboratory.idlab INNER JOIN orders ON orders.id_consulta = events.id WHERE id= '$id';");
 $sentencia->execute();
...
<br>
    ...
    <label for="psw"><b>Área del colaborador</b></label><span class="badge-warning">*</span>
    <select readonly required name="apppac" id="pati">
        <option><?php echo $d->grup; ?></option>
    </select>

    <label for="psw"><b>Número de Control</b></label><span class="badge-warning">*</span>
    <select readonly required name="apppac" id="pati">
    ...
    <label for="psw"><b>Medicamentos o Insumos</b></label><span class="badge-warning">*</span>
    <select readonly required name="" id="pati">
        <option><?php echo $d->total_products; ?></option>
    </select>
    <label for="email"><b>Costo</b></label><span class="badge-warning">*</span>

     <select readonly id="spe">
        <option><?php echo $d->total_price; ?></option>
    </select>
