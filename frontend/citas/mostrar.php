...
<head>
    ...
    <title>   SIE QDL | Listado de las Consultas</title>
</head>
<body>
...
$sentencia = $connect->prepare("SELECT events.id, events.title, patients.idpa, patients.grup, patients.numhs,patients.nompa, patients.apepa, doctor.idodc, doctor.ceddoc, doctor.nodoc, doctor.apdoc, laboratory.idlab, laboratory.nomlab, events.start, events.end, events.color, events.state,events.chec,events.monto FROM events INNER JOIN patients ON events.idpa = patients.idpa INNER JOIN doctor ON events.idodc = doctor.idodc INNER JOIN laboratory ON events.idlab = laboratory.idlab ORDER BY events.start DESC;");
 $sentencia->execute();
$data =  array();
if($sentencia){
  while($r = $sentencia->fetchObject()){
         ...
         <table id="example" class="responsive-table">
            ...
            <tbody>
                <?php foreach($data as $d):?>
                    <tr>
                        <td data-title="ID"><?php echo $d->start;?></td>
                        <td data-title="Cita"><?php echo $d->numhs ?></td>
                        <td scope="row"><?php echo $d->nompa ?>&nbsp;<?php echo $d->apepa ?></td>
                        <td data-title="Cita"><?php echo $d->grup ?></td>
              ...
              echo '<a title="Pago"  href="../citas/money.php?id='.$d->idpa.'" class="fa fa-money"></a>';
                ...
                <button name='delete_consulta' style="cursor: pointer;" class="fa fa-trash"></button>
                </form>
                        <?php endif; ?>
    ...
    <?php include_once '../../backend/php/delete_consulta.php' ?>
    <!-- NAVBAR -->
