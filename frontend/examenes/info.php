...
<head>
    ...
    <title>   SIE QDL | Información del Examen</title>
</head>
<body>
    ...
    <section id="content">
        ...
        <h1 class="title">Bienvenido <?php echo '<strong>'.$_SESSION['name'].'</strong>'; ?></h1>
            <ul class="breadcrumbs">
                ...
                <li class="divider">></li>
                <li><a href="#" class="active">Información del examen</a></li>
            </ul>
                   <!-- multistep form -->
<?php 
require '../../backend/bd/Conexion.php';
 $id = $_GET['id'];
 $sentencia = $connect->prepare("SELECT examenes.idex, examenes.fecha, examenes.nombre, examenes.status, examenes.observaciones, examenes.idodc, doctor.idodc, doctor.nodoc, doctor.apdoc FROM examenes INNER JOIN doctor ON examenes.idodc = doctor.idodc WHERE idex= '$id';");
 $sentencia->execute();
...
<br>
    ...
    <input readonly type="date" value="<?php echo $d->fecha; ?>" name="appini"required="">

    </select>
    <label for="psw"><b>Nombre</b></label><span class="badge-warning">*</span>
    <select readonly required name="apppac" id="pati">
        <option><?php echo $d->nombre; ?></option>
    </select>

    <label for="psw"><b>Estado</b></label><span class="badge-warning">*</span>
    <select readonly required name="apppac" id="pati">
        <option><?php echo $d->status; ?></option>
    </select>
    
    <label for="email"><b>observaciones</b></label><span class="badge-warning">*</span>
    <textarea name="appnam" style="height:200px" readonly placeholder="Write something.."><?php echo $d->observaciones; ?> </textarea>

    <label for="psw"><b>Nombre de quien realizó el examen</b></label><span class="badge-warning">*</span>
