...
<head>
    ...
    <title>   SIE QDL | Información del Accidente</title>
</head>
<body>
    ...
    <section id="content">
        ...
        <h1 class="title">Bienvenido <?php echo '<strong>'.$_SESSION['name'].'</strong>'; ?></h1>
            <ul class="breadcrumbs">
                ...
                <li class="divider">></li>
                <li><a href="#" class="active">Información del accidente</a></li>
            </ul>
                   <!-- multistep form -->
<?php 
require '../../backend/bd/Conexion.php';
 $id = $_GET['id'];
 $sentencia = $connect->prepare("SELECT laboratory.idlab, laboratory.nomlab, accidentes.id_acc, accidentes.fecha, 
                                accidentes.lesion, accidentes.incapacidad, accidentes.mecanismo_accidente, patients.idpa, 
                                patients.numhs, patients.nompa, patients.apepa, patients.grup 
                                FROM accidentes 
                                INNER JOIN patients ON accidentes.idpa = patients.idpa 
                                INNER JOIN laboratory ON accidentes.idlab = laboratory.idlab
                                WHERE id_acc= '$id';");
 $sentencia->execute();
...
<br>
    ...
    <input readonly type="datetime-local" value="<?php echo $d->fecha; ?>" name="appini"required="">
    ...
    <label for="email"><b>Lesión</b></label><span class="badge-warning">*</span>
    <textarea name="appnam" style="height:200px" readonly ><?php echo $d->lesion; ?> </textarea>
  
    

    <label for="psw"><b>Días de incapacidad</b></label><span class="badge-warning">*</span>
    <select readonly required name="" id="doc">
        <option><?php echo $d->incapacidad; ?></option>
    </select>

    <label for="psw"><b>Mecanismo del accidente</b></label><span class="badge-warning">*</span>
    <textarea name="appnam" style="height:200px" readonly ><?php echo $d->mecanismo_accidente; ?> </textarea>

    <label for="email"><b>Servicio de Atencion</b></label><span class="badge-warning">*</span>
    <textarea name="appnam"  readonly ><?php echo $d->nomlab; ?> </textarea>
