...
<head>
    ...
    <link rel="icon" href="..\backend\img\cccc.jpg" type="image/png">
                ...
                <div class="card">
                    <div class="head">
                        <div>
                                    ...
                                    <?php 
                                            $sql = "SELECT COUNT(*) total FROM doctor WHERE state = 1";
                                            $result = $connect->query($sql); //$pdo sería el objeto conexión
                                            $total = $result->fetchColumn();
...
$sentencia = $connect->prepare("SELECT * FROM doctor WHERE state = 1 ORDER BY idodc DESC LIMIT 10;");
 $sentencia->execute();
$data =  array();
if($sentencia){
    ...
    $data[] = $r;
     ...
     <?php if(count($data)>0):?>
         <table id="example" class="responsive-table">
            <thead>
                <tr>
                    <th scope="col">Personal registrados</th>
            ...
            <tbody>
                <?php foreach($data as $d):?>
                    <tr>
                        ...
                        <td data-title="Colaborador"><?php echo $d->nodoc ?>&nbsp;<?php echo $d->apdoc ?></td>
...
$sentencia = $connect->prepare("SELECT * FROM product ORDER BY stock ASC LIMIT 10;");
 $sentencia->execute();
$data =  array();
if($sentencia){
  while($r = $sentencia->fetchObject()){
    $data[] = $r;
     ...
     <?php if(count($data)>0):?>
         <table id="example" class="responsive-table">
            <thead>
                <tr>
                    <th scope="col">Medicamento con Stock Bajo</th>
            ...
            <tbody>
                <?php foreach($data as $d):?>
                    <tr>
                        ...
                        <td data-title="Colaborador"><?php echo $d->nompro ?></td>
                        <td data-title="Especialidad"><?php echo $d->stock ?></td>
...
$mesActual = $meses[$monthNumber];
?>
    <div class="content-data">
        <div class="head">
            <h3>Accidentes de <?php echo $mesActual; ?></h3>
        </div>
        <?php
        include_once '../../backend/bd/Conexion.php'; // Conectar a la base de datos

        // Nueva consulta para contar accidentes por área (grup)
        $consulta = $connect->prepare("
            SELECT p.grup, COUNT(a.id_acc) AS total_accidentes
            FROM accidentes a
            JOIN patients p ON a.idpa = p.idpa
            WHERE p.state = 1
                AND a.fecha >= DATE_FORMAT(CURDATE(), '%Y-%m-01')
                AND a.fecha < DATE_FORMAT(DATE_ADD(CURDATE(), INTERVAL 1 MONTH), '%Y-%m-01')
            GROUP BY p.grup
        ...
        foreach ($resultado as $fila) {
            $areas[] = $fila['grup'];
            $totales[] = $fila['total_accidentes'];
        }
        ?>
        <canvas id="accidentesPorArea" width="300" height="300"></canvas>
        <script>
            // Pasamos los datos de PHP a JavaScript
            var areas = <?php echo json_encode($areas); ?>;
            var totales = <?php echo json_encode($totales); ?>;

            var ctx = document.getElementById('accidentesPorArea').getContext('2d');
            var chart = new Chart(ctx, {
                type: 'bar', // Cambia a 'pie' si prefieres gráfico circular
                data: {
                    labels: areas,
                    datasets: [{
                        label: 'Accidentes por área',
                        data: totales,
                        backgroundColor: [
                            'rgba(255, 99, 132, 0.2)',
                            'rgba(54, 162, 235, 0.2)',
